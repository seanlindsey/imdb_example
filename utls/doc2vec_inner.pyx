        ...
        for l in range(lbl_length):
            ...
            for i in range(sentence_len):
                if codelens[i] == 0:
                    continue
                if hs:
                    fast_sentence_dbow_hs(points[i], codes[i], codelens[i], syn0, syn1, size, lbl_indexes[l], _alpha, work, tw, tl)
                if negative:
                    next_random = fast_sentence_dbow_neg(negative, table, table_len, syn0, syn1neg, size, indexes[i], lbl_indexes[l], _alpha, work, next_random, tw, tl)
